...
RSpec.configure do |config|
  ...
  ES_BIN = es_config['es_bin'].freeze
  ES_PORT = es_config['port'].freeze
  ES_VERSION = es_config['version'].freeze

  config.before :all, elasticsearch: true do
    unless Elasticsearch::Extensions::Test::Cluster.running?(command: ES_BIN,
                                                             on: ES_PORT.to_i,
                                                             version: ES_VERSION)
      Elasticsearch::Extensions::Test::Cluster.start(command: ES_BIN,
                                                     port: ES_PORT.to_i,
                                                     nodes: 1,
                                                     timeout: 120,
                                                     version: ES_VERSION)
